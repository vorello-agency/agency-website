...
const NEXT_STEPS = [
  {
    ...
    icon: Activity,
    iconClassName: "border-signal-orange/20 bg-signal-orange/10 text-signal-orange",
  },
  {
  ...
  },
  {
    ...
    icon: FileText,
    iconClassName: "border-signal-emerald/20 bg-signal-emerald/10 text-signal-emerald",
  },
] as const;

const StartSidebar: React.FC = () => {
  return (
    <div className="sm:pt-8 lg:col-span-4">
      <div className="space-y-6 lg:space-y-12">
        <div className="border-steel-grey/30 border-b pb-3">
          <h2 className="text-chrome-highlight text-lg font-bold tracking-wider uppercase">
            ¿Qué ocurrirá después?
          </h2>
        </div>

        <ul className="max-w-2xl space-y-6 lg:max-w-none">
          {NEXT_STEPS.map(({ title, description, detail, icon: Icon, iconClassName }) => (
            <li key={title} className="flex gap-4">
              ...
              <div className="flex flex-col gap-1">
                <h3 className="text-chrome-highlight text-sm font-semibold">{title}</h3>
                <p className="text-muted-foreground text-xs leading-relaxed text-balance sm:text-xs">
