...
export default function ContactSuccess({ name, reasonLabel }: ContactSuccessProps) {
  ...
  return (
    <div className="mx-auto mt-6 max-w-2xl animate-in rounded-2xl border border-steel-grey/30 bg-graphite-metal/30 p-8 text-center backdrop-blur-md fade-in zoom-in duration-500 sm:p-12">
      <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full border border-emerald-500/20 bg-emerald-500/10 text-emerald-400">
        <CheckCircle2 className="h-8 w-8" />
      </div>
      <span className="mb-2 block text-xs font-semibold uppercase tracking-widest text-emerald-400">
        {"// MENSAJE ENVIADO"}
      </span>
