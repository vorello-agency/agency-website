...
export const animateNavbarIconEnter = (label: string, svg: SVGElement) => {
  ...
  } else if (label === "Diferencial") {
    const paths = svg.querySelectorAll("path");
    if (paths.length > 0) {
      const tl = gsap.timeline({ overwrite: "auto" });
      tl.to(svg, { scale: 1.15, duration: 0.3, ease: "power2.out" }).fromTo(
        paths,
...
export const animateProcessIconEnter = (stepNum: string, svg: SVGElement) => {
  if (stepNum === "01") {
    ...
    if (circles.length >= 3 && paths.length >= 2) {
      ...
      const tl = gsap.timeline({ overwrite: "auto" });
      tl.fromTo(circles[0], { scale: 0.5 }, { scale: 1.3, duration: 0.3, ease: "back.out(2)" })
        .to(
          [paths[0], paths[1]],
          {
            strokeDashoffset: 0,
